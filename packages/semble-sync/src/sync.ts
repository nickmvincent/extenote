...
const DEFAULT_SLUG_LENGTH = 50;
const DEFAULT_SHORT_PREVIEW_LENGTH = 50;
...
/**
 * Compute a content hash for a SyncObject (based on its card representation).
 * Exported for use in testing and debugging.
 */
export function computeObjectHash(object: SyncObject): string | null {
 ...
 */
function getLocalId(object: SyncObject): string {
 ...
 */
function extractCollectionTags(object: SyncObject): string[] {
...
/**
 * Convert a SyncObject to a Semble URL card.
 ...
 */
function objectToCard(object: SyncObject): Omit<SembleCard, "$type"> | null {
...
export interface SyncContext {
  client: SembleClient;
  config: SembleConfig;
  extenoteConfig: SyncExtenoteConfig;
  cwd: string;
  project: string;
  state: SyncState;
  options: SyncOptions;
...
async function relinkCardsToCollections(
  ...
  const objectMap = new Map<string, SyncObject>();
  for (const obj of objects) {
...
async function pullCards(
  ctx: SyncContext,
  existingObjects: SyncObject[]
): Promise<{ pulled: number; skipped: number; errors: SyncResult["errors"]; newObjects: SyncObject[] }> {
  ...
  for (const { uri, cid, value: card } of cards) {
    ...
    try {
      ...
      // Create a minimal SyncObject for the result
...
export async function syncWithSemble(input: SyncInput): Promise<SyncResult> {
  ...
  let pullResult = { pulled: 0, skipped: 0, errors: [] as SyncResult["errors"], newObjects: [] as SyncObject[] };
