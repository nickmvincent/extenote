...
/**
 * Google Docs discussion plugin.
 *
 * Creates Google Docs for each content object, enabling comment-based discussions.
 *
 * Authentication options:
 * 1. Service account JSON file (recommended for automation)
 * 2. OAuth tokens via environment variables
 *
 * Required scopes:
...
export class GoogleDocsPlugin extends BaseDiscussionPlugin {
  ...
  async validate(config: GoogleDocsConfig): Promise<ValidationResult> {
    const errors: string[] = [];
    ...
    if (!credentialsPath && !accessToken) {
      errors.push(
        "credentialsPath, GOOGLE_APPLICATION_CREDENTIALS, or GOOGLE_ACCESS_TOKEN required"
      );
    }
  ...
  private async getAccessToken(config: GoogleDocsConfig): Promise<string> {
    ...
    const credentials = JSON.parse(credentialsRaw) as ServiceAccountCredentials;
    ...
    const tokenData = await response.json() as GoogleAccessToken;
  ...
  private getRoleForAccess(access: "view" | "comment" | "edit"): string {
    switch (access) {
      case "view":
        return "reader";
      case "comment":
  ...
  async create(options: CreateDiscussionOptions): Promise<CreateDiscussionResult> {
    const config = options.config as GoogleDocsConfig;
    ...
    try {
      ...
      if (config.folderId) {
        await fetch(
          `https://www.googleapis.com/drive/v3/files/${doc.documentId}?addParents=${config.folderId}`,
          {
            method: "PATCH",
            headers: {
              Authorization: `Bearer ${accessToken}`,
  ...
  async delete(link: DiscussionLink, config: GoogleDocsConfig): Promise<boolean> {
    if (!link.uri) {
      return false;
    }
    ...
    try {
      ...
      // Move to trash
      const response = await fetch(
        `https://www.googleapis.com/drive/v3/files/${link.uri}`,
        {
          method: "PATCH",
          headers: {
            Authorization: `Bearer ${accessToken}`,
